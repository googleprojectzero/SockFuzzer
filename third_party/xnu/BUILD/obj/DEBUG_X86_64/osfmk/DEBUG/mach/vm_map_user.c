...
#if ( __MigTypeCheck )
#if __MIG_check__Reply__vm_map_subsystem__
#if !defined(__MIG_check__Reply__vm_remap_new_t__defined)
#define __MIG_check__Reply__vm_remap_new_t__defined

mig_internal kern_return_t __MIG_check__Reply__vm_remap_new_t(__Reply__vm_remap_new_t *Out0P)
{

	typedef __Reply__vm_remap_new_t __Reply __attribute__((unused));
#if	__MigTypeCheck
	unsigned int msgh_size;
#endif	/* __MigTypeCheck */
	if (Out0P->Head.msgh_id != 3932) {
	    if (Out0P->Head.msgh_id == MACH_NOTIFY_SEND_ONCE)
		{ return MIG_SERVER_DIED; }
	    else
		{ return MIG_REPLY_MISMATCH; }
...
#endif /* !defined(__MIG_check__Reply__vm_remap_new_t__defined) */
#endif /* __MIG_check__Reply__vm_map_subsystem__ */
#endif /* ( __MigTypeCheck ) */


/* Routine vm_remap_new */
mig_external kern_return_t vm_remap_new
(
	vm_map_t target_task,
	vm_address_t *target_address,
	vm_size_t size,
...
#pragma pack(pop)
...
#ifdef	__MIG_check__Reply__vm_remap_new_t__defined
	kern_return_t check_result;
#endif	/* __MIG_check__Reply__vm_remap_new_t__defined */

	__DeclareSendRpc(3832, "vm_remap_new")
	...
	InP->Head.msgh_request_port = target_task;
	InP->Head.msgh_reply_port = mig_get_reply_port();
	InP->Head.msgh_id = 3832;
	InP->Head.msgh_reserved = 0;

	__BeforeSendRpc(3832, "vm_remap_new")
	msg_result = mach_msg(&InP->Head, MACH_SEND_MSG|MACH_RCV_MSG|MACH_MSG_OPTION_NONE, (mach_msg_size_t)sizeof(Request), (mach_msg_size_t)sizeof(Reply), InP->Head.msgh_reply_port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
	__AfterSendRpc(3832, "vm_remap_new")
	if (msg_result != MACH_MSG_SUCCESS) {
		__MachMsgErrorWithoutTimeout(msg_result);
	}
	if (msg_result != MACH_MSG_SUCCESS) {
...
#if	defined(__MIG_check__Reply__vm_remap_new_t__defined)
	check_result = __MIG_check__Reply__vm_remap_new_t((__Reply__vm_remap_new_t *)Out0P);
	if (check_result != MACH_MSG_SUCCESS) {
		mach_msg_destroy(&Out0P->Head);
		{ return check_result; }
	}
