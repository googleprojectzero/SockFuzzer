...
mig_internal novalue _Xvm_remap_new_external
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP);
...
#if ( __MigTypeCheck )
#if __MIG_check__Request__vm_map_subsystem__
#if !defined(__MIG_check__Request__vm_remap_new_external_t__defined)
#define __MIG_check__Request__vm_remap_new_external_t__defined

mig_internal kern_return_t __MIG_check__Request__vm_remap_new_external_t(__attribute__((__unused__)) __Request__vm_remap_new_external_t *In0P)
{

	typedef __Request__vm_remap_new_external_t __Request;
#if	__MigTypeCheck
	if (!(In0P->Head.msgh_bits & MACH_MSGH_BITS_COMPLEX) ||
	    (In0P->msgh_body.msgh_descriptor_count != 1) ||
	    (In0P->Head.msgh_size != (mach_msg_size_t)sizeof(__Request)))
...
#endif /* !defined(__MIG_check__Request__vm_remap_new_external_t__defined) */
#endif /* __MIG_check__Request__vm_map_subsystem__ */
#endif /* ( __MigTypeCheck ) */
...
#pragma pack(pop)
#endif
	typedef __Request__vm_remap_new_external_t __Request;
	typedef __Reply__vm_remap_new_external_t Reply __attribute__((unused));
	...
	Request *In0P = (Request *) InHeadP;
	Reply *OutP = (Reply *) OutHeadP;
#ifdef	__MIG_check__Request__vm_remap_new_external_t__defined
	kern_return_t check_result;
#endif	/* __MIG_check__Request__vm_remap_new_external_t__defined */
	...
	__DeclareRcvRpc(3832, "vm_remap_new_external")
	__BeforeRcvRpc(3832, "vm_remap_new_external")

#if	defined(__MIG_check__Request__vm_remap_new_external_t__defined)
	check_result = __MIG_check__Request__vm_remap_new_external_t((__Request *)In0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ MIG_RETURN_ERROR(OutP, check_result); }
#endif	/* defined(__MIG_check__Request__vm_remap_new_external_t__defined) */
	...
	OutP->RetCode = vm_remap_new_external(target_task, &In0P->target_address, In0P->size, In0P->mask, In0P->flags, In0P->src_tport.name, In0P->src_address, In0P->copy, &In0P->cur_protection, &In0P->max_protection, In0P->inheritance);
	vm_map_deallocate(target_task);
	if (OutP->RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, OutP->RetCode);
	}
	...
	OutP->Head.msgh_size = (mach_msg_size_t)(sizeof(Reply));
	__AfterRcvRpc(3832, "vm_remap_new_external")
}
...
const struct vm_map_subsystem vm_map_subsystem = {
	...
	(vm_address_t)0,
	{
		...
		{0, 0, 0, 0, 0, 0},
          ...
          { (mig_impl_routine_t) 0,
          (mig_stub_routine_t) _Xvm_remap_new_external, 14, 0, (routine_arg_descriptor_t)0, (mach_msg_size_t)sizeof(__Reply__vm_remap_new_external_t)},
	}
};
	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	if ((InHeadP->msgh_id > 3832) || (InHeadP->msgh_id < 3800) ||
