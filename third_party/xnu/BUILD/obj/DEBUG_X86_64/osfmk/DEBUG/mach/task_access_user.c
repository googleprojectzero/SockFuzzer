...
#if ( __MigTypeCheck )
#if __MIG_check__Reply__task_access_subsystem__
#if !defined(__MIG_check__Reply__check_task_access_with_flavor_t__defined)
#define __MIG_check__Reply__check_task_access_with_flavor_t__defined

mig_internal kern_return_t __MIG_check__Reply__check_task_access_with_flavor_t(__Reply__check_task_access_with_flavor_t *Out0P)
{

	typedef __Reply__check_task_access_with_flavor_t __Reply __attribute__((unused));
	if (Out0P->Head.msgh_id != 27102) {
	    if (Out0P->Head.msgh_id == MACH_NOTIFY_SEND_ONCE)
		{ return MIG_SERVER_DIED; }
	    else
		{ return MIG_REPLY_MISMATCH; }
...
#endif /* !defined(__MIG_check__Reply__check_task_access_with_flavor_t__defined) */
#endif /* __MIG_check__Reply__task_access_subsystem__ */
#endif /* ( __MigTypeCheck ) */


/* Routine check_task_access_with_flavor */
mig_external kern_return_t check_task_access_with_flavor
(
	mach_port_t task_access_port,
	int32_t calling_pid,
	uint32_t calling_gid,
...
#pragma pack(pop)
...
#ifdef	__MIG_check__Reply__check_task_access_with_flavor_t__defined
	kern_return_t check_result;
#endif	/* __MIG_check__Reply__check_task_access_with_flavor_t__defined */

	__DeclareSendRpc(27002, "check_task_access_with_flavor")
	...
	InP->Head.msgh_request_port = task_access_port;
	InP->Head.msgh_reply_port = mig_get_reply_port();
	InP->Head.msgh_id = 27002;
	InP->Head.msgh_reserved = 0;

	__BeforeSendRpc(27002, "check_task_access_with_flavor")
#if	(__MigKernelSpecificCode) || (_MIG_KERNELSPECIFIC_CODE_)
	msg_result = mach_msg_rpc_from_kernel(&InP->Head, (mach_msg_size_t)sizeof(Request), (mach_msg_size_t)sizeof(Reply));
#else
	msg_result = mach_msg(&InP->Head, MACH_SEND_MSG|MACH_RCV_MSG|MACH_MSG_OPTION_NONE, (mach_msg_size_t)sizeof(Request), (mach_msg_size_t)sizeof(Reply), InP->Head.msgh_reply_port, MACH_MSG_TIMEOUT_NONE, MACH_PORT_NULL);
#endif /* __MigKernelSpecificCode */
	__AfterSendRpc(27002, "check_task_access_with_flavor")
	if (msg_result != MACH_MSG_SUCCESS) {
		__MachMsgErrorWithoutTimeout(msg_result);
	}
	if (msg_result != MACH_MSG_SUCCESS) {
...
#if	defined(__MIG_check__Reply__check_task_access_with_flavor_t__defined)
	check_result = __MIG_check__Reply__check_task_access_with_flavor_t((__Reply__check_task_access_with_flavor_t *)Out0P);
	if (check_result != MACH_MSG_SUCCESS) {
#if	__MigKernelSpecificCode
		mach_msg_destroy_from_kernel(&Out0P->Head);
#endif	/* __MigKernelSpecificCode */
