...
mig_internal kern_return_t __MIG_check__Request__mach_vm_remap_new_external_t(__attribute__((__unused__)) __Request__mach_vm_remap_new_external_t *In0P)
{
...
#if	__MigTypeCheck
	if (In0P->src_tport.type != MACH_MSG_PORT_DESCRIPTOR ||
	    In0P->src_tport.disposition != 17)
		return MIG_TYPE_ERROR;
#endif	/* __MigTypeCheck */
...
#pragma pack(pop)
...
#if	defined(__MIG_check__Request__mach_vm_remap_new_external_t__defined)
	check_result = __MIG_check__Request__mach_vm_remap_new_external_t((__Request *)In0P);
	if (check_result != MACH_MSG_SUCCESS)
		{ MIG_RETURN_ERROR(OutP, check_result); }
#endif	/* defined(__MIG_check__Request__mach_vm_remap_new_external_t__defined) */
	...
	OutP->RetCode = mach_vm_remap_new_external(target_task, &In0P->target_address, In0P->size, In0P->mask, In0P->flags, In0P->src_tport.name, In0P->src_address, In0P->copy, &In0P->cur_protection, &In0P->max_protection, In0P->inheritance);
	vm_map_deallocate(target_task);
	if (OutP->RetCode != KERN_SUCCESS) {
		MIG_RETURN_ERROR(OutP, OutP->RetCode);
	}
	...
	OutP->cur_protection = In0P->cur_protection;

	OutP->max_protection = In0P->max_protection;
...
const struct mach_vm_subsystem mach_vm_subsystem = {
	...
	(vm_address_t)0,
	{
          ...
          { (mig_impl_routine_t) 0,
          (mig_stub_routine_t) _Xmach_vm_remap_new_external, 14, 0, (routine_arg_descriptor_t)0, (mach_msg_size_t)sizeof(__Reply__mach_vm_remap_new_external_t)},
	}
};
	...
	(mach_msg_header_t *InHeadP, mach_msg_header_t *OutHeadP)
{
	...
	if ((InHeadP->msgh_id > 4821) || (InHeadP->msgh_id < 4800) ||
